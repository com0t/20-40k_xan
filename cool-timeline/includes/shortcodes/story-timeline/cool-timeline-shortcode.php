	...
	public function cooltimeline_view($atts, $content = null) {
		...
		*/
		...
		if($layout!="horizontal"){  
			...
			$output .= '<div class="cool-timeline white-timeline ultimate-style ' .esc_attr($layout_wrp). ' ' .esc_attr($wrp_cls).' ' .$layout_cls.'">';
	...
	public function ctl_excerpt_more($more){
		...
		if($post->post_type == 'cool_timeline' && !is_single() ){
		    if($ctl_display_readmore=='yes'){
				...
				return '..<a class="read_more ctl_read_more" target="'.$target.'" href="'. get_permalink($post->ID) . '">'.$read_more_txt .'</a>';
			}  
	    }else{
		 	return $more;
	    }
